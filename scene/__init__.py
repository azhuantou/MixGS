...
class LargeScene(Scene):
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, load_vq=False, shuffle=True, resolution_scales=[1.0]):
        ...
        if self.load_vq:
            ...
            if args.block_id >= 0:
                self.gaussians.load_ply(os.path.join(args.source_path, "data_partitions", f"point_cloud_{args.block_id}.ply"))
                self.gaussians.spatial_lr_scale = self.cameras_extent
            else:
                self.gaussians.load_ply(self.pretrain_path)
                self.gaussians.spatial_lr_scale = self.cameras_extent
        else:
    ...
    def save(self, iteration, args=None):
        # point_cloud_path = os.path.join(self.model_path, "point_cloud/iteration_{}".format(iteration))
        point_cloud_path = os.path.join(args, "point_cloud/iteration_{}".format(iteration))
