...
def readColmapSceneInfo(path, images, eval, llffhold=None, partition=None):
    ...
    if eval:
        # llffhold = 83  # TODO hard code
        if llffhold != 'None':
            train_cam_infos = [c for idx, c in enumerate(cam_infos) if idx % llffhold != 0]
            test_cam_infos = [c for idx, c in enumerate(cam_infos) if idx % llffhold == 0]
        else:
            test_path = path[:-5] + 'val'
            test_cameras_extrinsic_file = os.path.join(test_path, "sparse/0", "images.bin")
            test_cameras_intrinsic_file = os.path.join(test_path, "sparse/0", "cameras.bin")
            ...
            reading_dir = "images" if images == None else images
            test_cam_infos_unsorted = readColmapCameras(cam_extrinsics=test_cam_extrinsics, cam_intrinsics=test_cam_intrinsics,
                                                   images_folder=os.path.join(test_path, reading_dir))
